...
wd = Path('/root/paddlejob/workspace/env_run/VideoSR/SeedVR')
...
@contextmanager
def nvtx(tag: str, only_rank0: bool = True):
    if only_rank0 and torch.cuda.current_device() == 0:
        with torch.cuda.nvtx.range(tag):
            yield
    else:
        yield
